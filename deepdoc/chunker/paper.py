...
class PaperChunker(BaseChunker):
    def chunk(self, document: DocumentStructure) -> List[Chunk]:
        ...
        if current_text:
            chunks.append(Chunk(text=current_text.strip(), chunk_index=chunk_idx, page_numbers=sorted(list(set(current_pages))), content_type="paper", metadata={"document_id": doc_id}))
